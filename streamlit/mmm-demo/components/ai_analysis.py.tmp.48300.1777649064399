...
def analyze_fit_with_llm(mmm, data: pd.DataFrame, api_key: str) -> str:
    ...
    # combined=True 時 dims 為 (sample, date_week)
    # combined=False 時 dims 為 (chain, draw, date_week)
    ...
    mape = float(np.mean(np.abs((y_true - y_pred_mean) / np.where(y_true == 0, 1, y_true))) * 100)
...
本次 MMM 模型採樣次數（draws）為 {draws_used}，低於最低建議值 500。
MCMC 在採樣不足時尚未收斂，所有指標（R²={r2:.2f}、MAPE={mape:.0f}%、覆蓋率={coverage:.0f}%）都是無效的，不代表模型或資料有問題。
...
擬合指標：
  - R²（決定係數）：{r2:.3f}（越接近 1 越好）
  - MAE（平均絕對誤差）：{mae:.2f}
  - MAPE（平均絕對百分比誤差）：{mape:.1f}%
  - 90% 預測區間覆蓋率：{coverage:.1f}%（理想值接近 90%）
  - MCMC 發散次數：{n_div}（理想值為 0）
  - 採樣次數（draws）：{draws_used}
